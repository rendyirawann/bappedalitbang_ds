...
use backend\models\search\TblJembatan2023Search;
...
/**
 * TblJembatan2023Controller implements the CRUD actions for TblJembatan2023 model.
 */
class TblJembatan2023Controller extends Controller
{
    /**
     * @inheritDoc
     */
    ...
    /**
     * Lists all TblJembatan2023 models.
     *
     * @return string
     */
    public function actionIndex()
    {
        $searchModel = new TblJembatan2023Search();
    ...
    /**
     * Displays a single TblJembatan2023 model.
     * @param int $id ID
     * @return string
    ...
    /**
     * Creates a new TblJembatan2023 model.
     * If creation is successful, the browser will be redirected to the 'view' page.
     * @return string|\yii\web\Response
     */
    public function actionCreate()
    {
        $model = new TblJembatan2023();
    ...
    /**
     * Updates an existing TblJembatan2023 model.
...
{
    $model = new UploadFormJembatan2023();
    ...
    if (Yii::$app->request->isPost) {
        ...
        if ($filePath) {
            ...
            foreach ($rows as $index => $row) {
                ...
                $jembatan = new TblJembatan2023();
                $jembatan->no = (int)$row[0]; // Column 0
                $jembatan->namaPekerjaan = !empty($row[1]) ? $row[1] : null; // Column 1
                $jembatan->penyedia = !empty($row[2]) ? $row[2] : null; // Column 2
                $jembatan->nilaiPagu = !empty($row[3]) ? $row[3] : null; // Column 3
                $jembatan->nilaiKontrak = !empty($row[4]) ? $row[4] : null; // Column 4
                $jembatan->nilaiKontrakAdd = !empty($row[5]) ? $row[5] : null; // Column 5
                $jembatan->noSpmk = !empty($row[6]) ? $row[6] : null; // Column 6
                $jembatan->noKontrak = !empty($row[7]) ? $row[7] : null; // Column 7
                $jembatan->noKontrakAdd = !empty($row[8]) ? $row[8] : null; // Column 8
                $jembatan->noPho = !empty($row[9]) ? $row[9] : null; // Column 9
                $jembatan->realisasi_meter = !empty($row[10]) ? $row[10] : null; // Column 10
                $jembatan->keterangan = !empty($row[11]) ? $row[11] : null; // Column 11
...
{
    $models = TblJembatan2023::find()->all();
...
public function actionExportXlsx()
{
    ...
    /**
     * Deletes an existing TblJembatan2023 model.
    ...
    /**
     * Finds the TblJembatan2023 model based on its primary key value.
     * If the model is not found, a 404 HTTP exception will be thrown.
     * @param int $id ID
     * @return TblJembatan2023 the loaded model
     * @throws NotFoundHttpException if the model cannot be found
     */
    protected function findModel($id)
    {
        if (($model = TblJembatan2023::findOne(['id' => $id])) !== null) {
            return $model;
        }
