...
use backend\models\search\TblJembatan2022Search;
...
/**
 * TblJembatan2022Controller implements the CRUD actions for TblJembatan2022 model.
 */
class TblJembatan2022Controller extends Controller
{
    /**
     * @inheritDoc
     */
    ...
    /**
     * Lists all TblJembatan2022 models.
     *
     * @return string
     */
    public function actionIndex()
    {
        $searchModel = new TblJembatan2022Search();
    ...
    /**
     * Displays a single TblJembatan2022 model.
     * @param int $id ID
     * @return string
    ...
    /**
     * Creates a new TblJembatan2022 model.
     * If creation is successful, the browser will be redirected to the 'view' page.
     * @return string|\yii\web\Response
     */
    public function actionCreate()
    {
        $model = new TblJembatan2022();
    ...
    /**
     * Updates an existing TblJembatan2022 model.
    ...
    {
        $model = new UploadFormJembatan2022();
        ...
        if (Yii::$app->request->isPost) {
            ...
            if ($filePath) {
                ...
                foreach ($rows as $index => $row) {
                    ...
                    $jembatan = new TblJembatan2022();
                    $jembatan->no = (int)$row[0]; // Column 0
                    $jembatan->namaPekerjaan = !empty($row[1]) ? $row[1] : null; // Column 1
                    $jembatan->realisasi_meter = !empty($row[2]) ? $row[2] : null; // Column 2
                    $jembatan->keterangan = !empty($row[3]) ? $row[3] : null; // Column 3
...
{
    $models = TblJembatan2022::find()->all();
...
public function actionExportXlsx()
{
    ...
    /**
     * Deletes an existing TblJembatan2022 model.
    ...
    /**
     * Finds the TblJembatan2022 model based on its primary key value.
     * If the model is not found, a 404 HTTP exception will be thrown.
     * @param int $id ID
     * @return TblJembatan2022 the loaded model
     * @throws NotFoundHttpException if the model cannot be found
     */
    protected function findModel($id)
    {
        if (($model = TblJembatan2022::findOne(['id' => $id])) !== null) {
            return $model;
        }
