...
use backend\models\search\TblIndividu2023Search;
...
/**
 * TblIndividu2023Controller implements the CRUD actions for TblIndividu2023 model.
 */
class TblIndividu2023Controller extends Controller
{
    /**
     * @inheritDoc
     */
    ...
    /**
     * Lists all TblIndividu2023 models.
     *
     * @return string
     */
    public function actionIndex()
    {
        $searchModel = new TblIndividu2023Search();
    ...
    /**
     * Displays a single TblIndividu2023 model.
     * @param int $id ID
     * @return string
    ...
    /**
     * Creates a new TblIndividu2023 model.
     * If creation is successful, the browser will be redirected to the 'view' page.
     * @return string|\yii\web\Response
     */
    public function actionCreate()
    {
        $model = new TblIndividu2023();
    ...
    /**
     * Updates an existing TblIndividu2023 model.
...
{
    $model = new UploadFormIndividu2023();
    ...
    if (Yii::$app->request->isPost) {
        ...
        if ($filePath) {
            ...
            foreach ($rows as $index => $row) {
                ...
                $individu = new TblIndividu2023();
                $individu->no = $row[0]; // Column 0
                $individu->kode_kegiatan = !empty($row[1]) ? $row[1] : null; // Column 1
                $individu->kode_sub = !empty($row[2]) ? $row[2] : null; // Column 2
                $individu->kode_rekening = !empty($row[3]) ? $row[3] : null; // Column 3
                $individu->namaKegiatan = $row[4]; // Column 4
                $individu->kecamatan = $row[5]; // Column 5
                $individu->satuan = $row[6]; // Column 6
                $individu->hargaSatuan = (int)$row[7]; // Column 7
                $individu->jumlah = (int)$row[8]; // Column 8
                $individu->keterangan = $row[9]; // Column 9
...
{
    $models = TblIndividu2023::find()->all();
...
public function actionExportXlsx()
{
    ...
    /**
     * Deletes an existing TblIndividu2023 model.
    ...
    /**
     * Finds the TblIndividu2023 model based on its primary key value.
     * If the model is not found, a 404 HTTP exception will be thrown.
     * @param int $id ID
     * @return TblIndividu2023 the loaded model
     * @throws NotFoundHttpException if the model cannot be found
     */
    protected function findModel($id)
    {
        if (($model = TblIndividu2023::findOne(['id' => $id])) !== null) {
            return $model;
        }
