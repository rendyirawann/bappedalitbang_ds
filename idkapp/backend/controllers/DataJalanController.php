...
use backend\models\search\DataJalanSearch;
...
/**
 * DataJalanController implements the CRUD actions for DataJalan model.
 */
class DataJalanController extends Controller
{
    ...
    /**
     * Lists all DataJalan models.
     *
     * @return string
     */
    public function actionIndex()
    {
        $searchModel = new DataJalanSearch();
    ...
    /**
     * Displays a single DataJalan model.
     * @param int $id ID
     * @return string
    ...
    /**
     * Updates an existing DataJalan model.
    ...
    if (Yii::$app->request->isPost) {
        ...
        if ($filePath) {
            ...
            foreach ($rows as $index => $row) {
                ...
                $individu = new DataJalan();
                $individu->kodeKecamatan = (int)$row[0]; // Column 0
                $individu->namaJalan = !empty($row[1]) ? $row[1] : null; // Column 6
...
private function parseDecimal($value)
{
    ...
    /**
     * Deletes an existing DataJalan model.
    ...
    /**
     * Finds the DataJalan model based on its primary key value.
     * If the model is not found, a 404 HTTP exception will be thrown.
     * @param int $id ID
     * @return DataJalan the loaded model
     * @throws NotFoundHttpException if the model cannot be found
     */
    protected function findModel($id)
    {
        if (($model = DataJalan::findOne(['id' => $id])) !== null) {
            return $model;
        }
