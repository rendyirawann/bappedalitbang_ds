            ...
            <div class="collapse" id="collapseProfile">
                <div class="card border-primary shadow-sm">
                    <div class="card-body">
                        ...
                        <?php /* Hapus <p class="card-text"> deskripsi profil jika sudah tidak relevan atau sesuaikan */ ?>
                        ...
                        <?php if (!empty($profils)): ?>
                            <hr> <?php // Pemisah visual 
                                    ?>
                            <?php /* <h6 class="mt-3">Dokumentasi/Gambar Terkait Profil:</h6> */ // Judul ini bisa dihilangkan jika namaFile di card-header sudah cukup 
                            ?>
                            <div class="row mt-2">
                                <?php foreach ($profils as $key => $profilItem): ?>
                                    <?php if (!empty($profilItem->file)):
                                        $imageUrl = Yii::getAlias('@web/uploads/profil/') . Html::encode($profilItem->file);
                                        $imageTitle = !empty($profilItem->namaFile) ? $profilItem->namaFile : 'Gambar Profil ' . ($key + 1);
                                    ?>
                                        <div class="col-lg-12 mb-3"> <?php // Setiap gambar dalam col-lg-12 
                                            ...
                                            <div class="card shadow-sm">
                                                ...
                                                <div class="card-body text-center p-2">
                                                    <a href="<?= $imageUrl ?>"
                                                        data-lightbox="profil-gallery" <?php // Nama grup untuk galeri lightbox 
                                                                                        ?>
                                                        data-title="<?= Html::encode($imageTitle) ?>" <?php // Judul yang akan tampil di lightbox 
                                                                                                        ?>
                                                        title="<?= Html::encode($imageTitle) ?>" <?php // Tooltip browser 
                                                                                                    ?>>
                                                        <img src="<?= $imageUrl ?>"
                                                            alt="<?= Html::encode($imageTitle) ?>"
                                                            class="img-fluid rounded"
                                                            style="max-height: 400px; width: auto; max-width: 100%; cursor: pointer;"> <?php // Mengurangi max-height sedikit, tambahkan cursor pointer 
                                                                                                                                        ?>
                                                    </a>
